...
private:
    ...
    // Flat data block structure with no pointer overhead or internal vectors
    struct DataBlock {
        int keys[MAX_KEYS];                 
    ...
    // Computes total elements in a subtree using flat block math
    ...
    int count_keys_less_than(int block_idx, int val) const {
        ...
        // 100% cache-local linear scan. No dynamic pointers or arrow (->) lookups.
    ...
    // Handles splitting overloaded blocks during insertion safely without dangling references
    void split_full_block(int parent_idx, int i, int child_idx) {
        DataBlock new_block;
        // Read directly from safe indexes instead of using unsafe references across push_backs
        ...
        // Fetching structural fields dynamically via indexes protects against vector reallocations
        ...
        // Grab the exact median key tracking safely from child layout
        storage_pool_[parent_idx].keys[i] = storage_pool_[child_idx].keys[BLOCK_DEGREE - 1];
        storage_pool_[parent_idx].num_keys++;
    }

    // Inserts a value into a block that is guaranteed to have space
...
public:
    ...
    void insert(int val) {
        if (storage_pool_[root_index_].num_keys == MAX_KEYS) {
            ...
            new_root.child_indices[0] = root_index_; // Fix: properly assign the child index array elements
    ...
    // Total elements in the multiset: O(N) internal node traversal
    size_t size() const {
        return compute_subtree_size(root_index_);
    }

    // Check if the multiset is empty: O(1)
    bool empty() const {
        return storage_pool_[root_index_].num_keys == 0;
    }
    ...
    // Returns number of elements strictly greater than 'val': O(log N)
    int upper_bound_count(int val) const {
        return compute_subtree_size(root_index_) - lower_bound_count(val + 1);
    }
};
